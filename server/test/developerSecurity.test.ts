...
describe('developer platform security primitives',()=>{
  it('generates recognizable high-entropy keys and only a keyed verifier is suitable for storage',()=>{const a=generateApiKey(),b=generateApiKey(),secret=a.plaintext.slice(`gp_test_${a.publicId}_`.length);expect(a.plaintext).toMatch(/^gp_test_[A-Za-z0-9_-]{12}_[A-Za-z0-9_-]{43}$/);expect(a.plaintext).not.toBe(b.plaintext);const verifier=apiKeyVerifier(a.plaintext,'p'.repeat(32));expect(verifier).toHaveLength(64);expect(verifier).not.toContain(a.plaintext);expect(a.fingerprint).toBe(`gp_test_${a.publicId}_...${secret.slice(-4)}`);expect(a.fingerprint).not.toContain(secret);});
