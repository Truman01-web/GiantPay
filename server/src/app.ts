...
export async function buildApp(config: Config, db: Db, provider: PaymentProvider = createPaymentProvider(config), workersReady:()=>boolean=()=>true, suppliedRateLimits?:RateLimitStore) {
  ...
  const paymentCreationControl=operationalControlGuard(db,'PAYMENT_CREATION_PAUSED'),refundMutationControl=operationalControlGuard(db,'REFUND_MUTATIONS_PAUSED');
  ...
  app.post('/v1/checkout/:token/submit', {preHandler:paymentCreationControl}, async (request, reply) => {
