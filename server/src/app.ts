...
export async function buildApp(config: Config, db: Db) {
  ...
  app.post('/v1/auth/register', async (request, reply) => {
    ...
    try {
      await db.query('INSERT INTO merchants(id,name) VALUES($1,$2)', [merchantId, body.businessName]);
      await db.query(`INSERT INTO users(id,merchant_id,name,email,password_hash,role,permissions) VALUES($1,$2,$3,$4,$5,'OWNER',$6)`, [userId,merchantId,body.businessName,body.email,passwordHash,['payments:read','payments.links:manage','payments.refunds:request','settings:manage']]);
  ...
  const refundJoin=`SELECT r.*,p.reference payment_reference,p.currency,ru.name requested_by_name,au.name approved_by_name FROM refunds r JOIN payments p ON p.id=r.payment_id JOIN users ru ON ru.id=r.requested_by LEFT JOIN users au ON au.id=r.approved_by`;
