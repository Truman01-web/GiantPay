...
export class WebhookDeliveryWorker {
  ...
  private async finish(row:any,error:DeliveryError|null,status?:number,excerpt?:string){await transaction(this.db,async c=>{await c.query(`INSERT INTO webhook_delivery_attempts(id,delivery_id,attempt_number,http_status,error_class,response_excerpt,completed_at) VALUES($1,$2,$3,$4,$5,$6,now()) ON CONFLICT(delivery_id,attempt_number) DO NOTHING`,[newId('wha'),row.id,row.attempt_count,status??null,error?.kind??null,excerpt??null]);if(!error){await c.query(`UPDATE webhook_deliveries SET status='SUCCEEDED',succeeded_at=now(),locked_at=NULL,last_http_status=$2,last_response=$3,last_error_class=NULL,updated_at=now() WHERE id=$1`,[row.id,status,excerpt]);operationalMetrics.increment('worker_jobs_total',{worker:'webhook',outcome:'succeeded'});}else{const terminal=!error.retryable||Number(row.attempt_count)>=Number(row.max_attempts);await c.query(`UPDATE webhook_deliveries SET status=CASE WHEN NOT $2 OR attempt_count>=max_attempts THEN 'FAILED' ELSE 'PENDING' END,next_attempt_at=now()+(LEAST(900,power(2,attempt_count))||' seconds')::interval,locked_at=NULL,last_http_status=$3,last_error_class=$4,last_response=$5,updated_at=now() WHERE id=$1`,[row.id,error.retryable,status??null,error.kind,excerpt??null]);operationalMetrics.increment('worker_jobs_total',{worker:'webhook',outcome:terminal?'terminal':'retry'});}});}
